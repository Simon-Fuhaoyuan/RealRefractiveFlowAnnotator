...
class FlowCalibrator:
    ...
    def saveFlow(self, flow_x, flow_y, rho):
        ...
        imageio.imsave(os.path.join(args.out_dir, args.out_name + '.png'), self.flowWithRho(flow_color, rho))
        utils.writeFlowBinary(flow, os.path.join(args.out_dir, args.out_name + '.flo'))

def readImgOrLoadNpy():
    if args.reload and os.path.exists(os.path.join(args.in_dir, 'imgs.npy')):
        if not args.mute:
            print('Loading imgs.npy')
        imgs = np.load(os.path.join(args.in_dir, 'imgs.npy'))
    else:
        exts = ['.png', '.jpg']
        ...
        np.save(os.path.join(args.in_dir, 'imgs.npy'), imgs)
    return imgs
